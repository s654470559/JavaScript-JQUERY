TreeGrid = function (_config) {
    ...
    for (var i = 0; i < _rows.length; i++) {
      ...
      for (var j = 0; j < _cols.length; j++) {
        ...
        if (j == folderColumnIndex) {
          if (_level > 1 && !row.children) {
            if (row.tree_last) {
              s += " style='text-indent:" + (parseInt((_config.indentation || "20")) * (_level - 1)) + "px;'> <span class='metro'>&nbsp;└</span>  ";
            } else {
              s += " style='text-indent:" + (parseInt((_config.indentation || "20")) * (_level - 1)) + "px;'> <span class='metro'>&nbsp;├</span>  ";
            }
          } else {
        ...
        if (j == folderColumnIndex) {
          ...
          } else if (_level == 1 && (!row.children || row.children.length == 0)) {
            s += "<img folder='Y' trid='TR" + id + "' src='" + folderCloseIcon + "' class='image_hand only_level1'>";
          }
          // else {
          //   s += "<img src='" + defaultLeafIcon + "' class='image_nohand'>"; // 如果需要默认图标
          // }
        }
  ...
  init = function () {
    ...
    // 将单击事件绑定到tr标签
    __root.find("tr").not(".header").bind("click", function (event) {
    ...
    // 展开、关闭下级节点
    __root.find("img[folder='Y']").bind("click", function (event) {
      event.stopPropagation();
    ...
    if (!_config || !_config.data || _config.data.length == 0) {
      console.log($(".renderTable .table"));
      $(".renderTable tbody").append(`
    <tr><td  colspan="9" class="data_empty">表中数据为空</td></tr>
    `)
    }
  }
...
// 公共静态变量
TreeGrid.FOLDER_OPEN_ICON = "../img/pull-down.png";
TreeGrid.FOLDER_CLOSE_ICON = "../img/pull-down.png";
